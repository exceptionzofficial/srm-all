...
import {
    ...
    FiFileText, FiBook, FiUsers, FiUpload, FiCheck, FiChevronRight, FiChevronLeft
} from 'react-icons/fi';
import {
...
const EmployeeForm = () => {
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            const payload = {
                ...formData,
                role: 'EMPLOYEE',
                addedBy: id ? undefined : 'Branch Manager', // Branch Portal Logic
                statutoryDetails: {
    ...
    useEffect(() => {
        ...
        if (verification.resendTimer > 0) {
            interval = setInterval(() => {
                setVerification(prev => ({ ...prev, resendTimer: prev.resendTimer - 1 }));
            }, 1000);
        } else {
    ...
    const handleSendEmailOTP = async () => {
        ...
        setVerification(p => ({ ...p, canResend: false, resendTimer: 30 }));
            ...
            <div className="bg-white rounded-2xl shadow-lg border border-gray-100 p-8 min-h-[500px] mb-8 animate-fade-in relative">
                ...
                {currentStep === 1 && (
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div className="form-group">
                            ...
                            <select className="input" value={formData.branchId} onChange={e => handleChange('branchId', e.target.value)}><option value="">Select Branch</option>{branches.map(b => <option key={b.branchId} value={b.branchId}>{b.name}</option>)}</select>
                        </div>
                        ...
                        {/* Employee ID is auto-generated, so we don't need an input for it */}
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div className="form-group">
                            ...
                            <div className="flex flex-col gap-2">
                                <div className="flex gap-2"><input type="text" className={`input flex-1 ${verification.mobileVerified ? 'bg-green-50' : ''}`} value={formData.personalMobile} onChange={e => handleChange('personalMobile', e.target.value)} disabled={verification.mobileVerified} />{!verification.mobileVerified && !verification.mobileOtpSent && <button onClick={handleSendMobileOTP} className="px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700">Verify</button>}</div>
                                {verification.mobileOtpSent && !verification.mobileVerified && (
                                    <div className="flex flex-col gap-2">
                                        <div className="flex gap-2">
                                            <input type="text" className="input flex-1" placeholder="OTP" value={verification.mobileOtp} onChange={e => setVerification(p => ({ ...p, mobileOtp: e.target.value }))} />
                                        ...
                                        <button
                                            ...
                                            className={`text-sm ${verification.canResend ? 'text-blue-600 hover:text-blue-800' : 'text-gray-400'} text-right`}
                                        >
                                            {verification.canResend ? 'Resend OTP' : `Resend in ${verification.resendTimer}s`}
                                        </button>
                                    </div>
                                )}
                            </div>
                        ...
                        <div className="form-group">
                            ...
                            <div className="flex flex-col gap-2">
                                <div className="flex gap-2"><input type="text" className={`input flex-1 ${verification.emailVerified ? 'bg-green-50' : ''}`} value={formData.personalEmail} onChange={e => handleChange('personalEmail', e.target.value)} disabled={verification.emailVerified} />{!verification.emailVerified && !verification.emailOtpSent && <button onClick={handleSendEmailOTP} className="px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700">Verify</button>}</div>
                                {verification.emailOtpSent && !verification.emailVerified && (
                                    <div className="flex flex-col gap-2">
                                        <div className="flex gap-2">
                                            <input type="text" className="input flex-1" placeholder="OTP" value={verification.emailOtp} onChange={e => setVerification(p => ({ ...p, emailOtp: e.target.value }))} />
                ...
                {currentStep === 8 && (
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div className="form-group"><label className="label">Photo</label><input type="file" accept="image/*" className="input" onChange={e => handleFileChange('photo', e)} /></div>
                        <div className="form-group"><label className="label">Aadhar Card</label><input type="file" className="input" onChange={e => handleFileChange('aadhar', e)} /></div>
                        <div className="form-group"><label className="label">PAN Card</label><input type="file" className="input" onChange={e => handleFileChange('pan', e)} /></div>
                        <div className="form-group"><label className="label">Marksheet</label><input type="file" className="input" onChange={e => handleFileChange('marksheet', e)} /></div>
                        <div className="form-group"><label className="label">Driving License</label><input type="file" className="input" onChange={e => handleFileChange('license', e)} /></div>
                    </div>
                )}
